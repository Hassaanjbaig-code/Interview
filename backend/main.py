...
def read_organization(organization_id: int, db: Session = Depends(get_db)) -> OrganizationsBase:
...
def create_organization(organization: OrganizationsCreate, db: Session = Depends(get_db)) -> OrganizationsBase:
...
def create_contact(contact: ContactsCreate, db: Session = Depends(get_db)) -> ContactsBase:
...
def update_organization(
    ...
    db_organization = db.query(Organizations).filter(Organizations.id == organization_id).first()
...
def update_contact(contact_id: int, contact: ContactsUpdate, db: Session = Depends(get_db)) -> ContactsBase:
...
def delete_organization(organization_id: int, db: Session = Depends(get_db)) -> OrganizationDelete:
    db_organization = db.query(Organizations).filter(Organizations.id == organization_id).first()
    if db_organization is None:
        raise HTTPException(status_code=404, detail="Organization not found")

    db_contacts = db.query(Contacts).filter(Contacts.organization_id == organization_id).all()
